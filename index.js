...
const queryKumo = function() {
    ...
    kumo.getRoomList().forEach(roomLabel => {
        ...
        kumo.getStatus(address).then((result) => {
            if (_.isNil(result.r) || _.isNil(result.r.indoorUnit) || _.isNil(result.r.indoorUnit.status)) {
                logging.error('bad result body: ' + JSON.stringify(result))
                return
            }

            const status = result.r.indoorUnit.status
            logging.info('Room: ' + roomLabel)
